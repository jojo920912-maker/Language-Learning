<template>
  <div class="auth-page">
    <div class="auth-card card">
      <!-- Loading -->
      <div v-if="state === 'loading'" class="center-section">
        <div class="big-spinner" />
        <p class="loading-text">驗證連結中...</p>
      </div>

      <!-- Invalid / expired link -->
      <div v-else-if="state === 'invalid'" class="center-section">
        <div class="state-icon">⚠️</div>
        <h2>連結無效或已過期</h2>
        <p class="state-desc">{{ errorMsg || '此連結可能已被使用或已過期，請重新申請。' }}</p>
        <RouterLink to="/forgot-password" class="btn btn-primary">重新申請 →</RouterLink>
      </div>

      <!-- Reset password form -->
      <div v-else-if="state === 'reset-form'">
        <div class="auth-logo">
          <span>🔐</span>
          <h1>重設密碼</h1>
          <p>為 <strong>{{ email }}</strong> 設定新密碼</p>
        </div>
      ...
      <!-- Success -->
      <div v-else-if="state === 'done'" class="center-section">
        <div class="state-icon">🎉</div>
        <h2>密碼重設成功！</h2>
        <p class="state-desc">你的密碼已更新，請使用新密碼登入。</p>
...
const state = ref<'loading' | 'invalid' | 'reset-form' | 'done'>('loading')
const oobCode = ref('')
const email = ref('')
const newPw = ref('')
const confirmPw = ref('')
...
async function onSubmit() {
  ...
  loading.value = true
  try {
    await confirmPasswordReset(auth, oobCode.value, newPw.value)
    state.value = 'done'
  } catch (e: any) {
    const code = e?.code ?? ''
...
.center-section { text-align: center; padding: 12px 0; }
.state-icon { font-size: 3rem; margin-bottom: 16px; }
.center-section h2 { font-size: 1.3rem; margin-bottom: 10px; }
.state-desc { font-size: 0.9rem; color: var(--text-muted); margin-bottom: 24px; }
.center-section .btn { display: inline-flex; }
.big-spinner { width: 40px; height: 40px; border: 3px solid var(--color-latte); border-top-color: var(--accent); border-radius: 50%; animation: spin 0.8s linear infinite; margin: 20px auto; }
